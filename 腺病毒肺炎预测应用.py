...
# 并发症输入
complication = st.sidebar.selectbox("并发症", ["无", "有"], index=0)
complication = 1 if complication == "有" else 0
...
if st.sidebar.button("预测") and model_loaded:
    ...
    def logistic_prediction(row):
        ...
        P = (0.810 * row['热峰']) + (0.705 * row['并发症']) + (0.890 * row['基础疾病']) - 30.205
        prob = 1 / (1 + np.exp(-P))
    ...
    st.write(f"并发症: {'有' if complication == 1 else '无'}")

# 模型性能信息
st.subheader("模型性能")
performance_data = {
    "模型": ["XGBoost", "LightGBM", "RandomForest", "Logistic回归"],
    "准确率": ["55.00%", "60.00%", "55.00%", "60.00%"],
    "精确率": ["54.55%", "60.00%", "55.56%", "55.56%"],
    "召回率": ["60.00%", "60.00%", "50.00%", "100.00%"],
    "F1分数": ["0.5714", "0.6000", "0.5263", "0.7143"]
}
